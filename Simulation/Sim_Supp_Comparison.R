...
set.seed(123)
sim_data <- readRDS("Data/Sim_Data_Main_Comparison.rds")
...
estimate_p2sls <- function(data) {
  fit_w_xy <- lm(W ~ Z + X + V, data = data)
  model_y_xy <- lm(Y ~ X + fit_w_xy$fitted.values + V, data = data)
  beta_xy <- coef(model_y_xy)["X"]
  
  fit_z_yx <- lm(Z ~ W + Y + V, data = data)
  model_x_yx <- lm(X ~ Y + fit_z_yx$fitted.values + V, data = data)
  beta_yx <- coef(model_x_yx)["Y"]
...
create_plot <- function(df, direction, true_value, manual_ylim) {
  ...
  df$estimator <- factor(df$estimator, levels = c("OLS", "IV", "Bi_TSLS", "P2SLS"))
  df$N <- factor(df$N)
  ...
  ggplot(df, aes(x = N, y = !!sym(beta_var), fill = estimator)) +
    ...
    scale_fill_manual(values = c("#a6cee3", "#b2df8a", "#fb9a99", "#fdbf6f"), labels = c("OLS", "IV", "Bi-TSLS", "P2SLS")) +
...
plot_xy <- create_plot(df_results, "XY", 0.5, manual_ylim = c(-0.8, 1.2))
plot_yx <- create_plot(df_results, "YX", -0.5, manual_ylim = c(-1, 0.1))

combined_plot <- grid.arrange(plot_xy, plot_yx, nrow = 2)
ggsave("Figure/Fig_Supp_Comparison.eps", combined_plot, width = 18, height = 12, units = "in", dpi = 600, device = "eps")
