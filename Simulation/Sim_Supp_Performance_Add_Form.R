...
estimate_bitsls <- function(data) {
  # Form 1
  fit_w <- lm(W ~ Z + V + I(Z^2) + I(V^2) + I(Z*V), data = data)
  fit_z <- lm(Z ~ W + V + I(W^2) + I(V^2) + I(W*V), data = data)
  
  # Form 2
  #fit_w <- lm(W ~ Z + V + I(Z^2) + I(Z^3) + I(V^2) + I(V^3) + I(Z*V), data = data)
  #fit_z <- lm(Z ~ W + V + I(W^2) + I(W^3) + I(V^2) + I(V^3) + I(W*V), data = data)
  
  # Form 3
  #fit_w <- lm(W ~ Z + V + I(Z^2) + I(V^2) + I(Z*V) + I(Z^2*V) + I(Z*V^2) + I(Z^2*V^2), data = data)
  #fit_z <- lm(Z ~ W + V + I(W^2) + I(V^2) + I(W*V) + I(W^2*V) + I(W*V^2) + I(W^2*V^2), data = data)
...
ggsave("Figure/Fig_Supp_Form1.eps", combined_plot, width = 18, height = 12, units = "in", dpi = 600, device = "eps")

#ggsave("Figure/Fig_Supp_Form2.eps", combined_plot, width = 18, height = 12, units = "in", dpi = 600, device = "eps")

#ggsave("Figure/Fig_Supp_Form3.eps", combined_plot, width = 18, height = 12, units = "in", dpi = 600, device = "eps")