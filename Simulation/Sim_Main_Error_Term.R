...
set.seed(123)
sim_data <- readRDS("Data/Sim_Data_Error_Term.rds")
...
create_plot <- function(df, direction, true_value, manual_ylim, sample_size = 5000) {
  ...
  main_title <- if(direction == "XY") {
    bquote(hat(beta)[X %->% Y] ~ "across different sample sizes and methods in dependent scenarios")
  } else {
    bquote(hat(beta)[Y %->% X] ~ "across different sample sizes and methods in dependent scenarios")
  }
...
plot_yx <- create_plot(df_results, "YX", -0.5, manual_ylim = c(-0.7, 0.1))

combined_plot <- grid.arrange(plot_xy, plot_yx, nrow = 2)
ggsave("Figure/Fig_Supp_Error_Term.eps", combined_plot, width = 18, height = 12, units = "in", dpi = 600, device = "eps")